"""The app's icon: a reticle, drawn in pure Python (no image library).

Shared by two consumers: the PyInstaller build step (``packaging/make_icon.py``
writes this to an .ico for the executable's resource, per the build's
"no binary blobs, no extra dependencies" policy) and the running app itself
(``ui/app.py`` sets it as the window/taskbar icon), so a source checkout
looks the same as the shipped .exe instead of showing a generic Python icon.
"""
